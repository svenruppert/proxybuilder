...
public class DynamicProxyBuilder<I, T extends I> implements HasLogger {
...
//          .withPostActions(postActionList)
  ...
  private void buildLoggingProxy() {
    final InvocationHandler invocationHandler = new InvocationHandler() {
      private final T original = DynamicProxyBuilder.this.original;
      private final Class<?> loggerClass = (clazzOrigin == null)
                                           ? original.getClass()
                                           : clazzOrigin;
      ...
      public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        final int length = (args == null)
                           ? 0
                           : args.length;
        if (DynamicProxyBuilder.this.logger(loggerClass).isInfoEnabled()) {
          DynamicProxyBuilder.this.logger(loggerClass).info(
